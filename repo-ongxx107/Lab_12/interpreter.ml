...
let rec exec (s: stmt) (stt: state) : state =
  ...
  | Skip -> stt
  | IfThenElse (cond, s1, s2) -> (match eval cond stt with
  ...
  | For (nm, e1, e2, body) ->
        ...
        | Int v1 -> (match eval e2 stt with
                          | Int v2 -> if v1 <= v2 then exec (Seq (body, For (nm, Add( Val(Int v1), Val(Int 1)), Val(Int v2), body))) ((nm, Int v1):: stt)
                                                        else stt
